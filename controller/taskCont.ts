...
export const addNewTask = async (req, res) => {
    try {
        ...
        if (
            ...
            await newTask.save();
            res.send({
                currentUsersTasks: await task.find({ ownerId: userId })
            });
        }
    } catch (error) {
        console.error(error);
    ...
    try {
        ...
        if (taskCheck?.checked === true) {
            await task.updateOne(
                { _id: _id, ownerId: ownerId },
                { timeChecked: timeChecked, checked: false }
            );
            const currentUsersTasks = await task.find({ ownerId: ownerId });
            res.send({ currentUsersTasks });
            return;
        ...
        const checkTask = await task.findOneAndUpdate(
            { _id: _id, ownerId: ownerId },
            { timeChecked: timeChecked, checked: true }
        );
        const currentUsersTasks = await task.find({ ownerId: ownerId });
        res.send({ currentUsersTasks });
    } catch (error) {
...
export const getUrgencies = async (req, res) => {
    ...
    const mediumUrgency = await task.find({
        ownerId: userId,
        urgency: 'medium'
    });
